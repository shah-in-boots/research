...
prog <-
	googledrive::drive_get("research_log") |>
	googlesheets4::read_sheet(sheet = "progress_sheet")
...
# Second version
tbl_status_updates <- function(data) {
...
```{r}
#| label: Week 2 updates
updates <-
	updates |>
	mutate(Status = case_when(
		Project == p$mental_stress ~
			"Working on tables and figures",
		Project == p$af_registry ~
			"Revise data dictionary",
		Project == p$pert ~ 
			"TBD",
		Project == p$grants ~ 
			"Considering isoproterenol washout",
		Project == p$study ~
			"TBD",
		Project == p$clinic ~ 
		...
		Project == p$signals ~
			"TBD",
		Project == p$biobank ~
			"Continuous ECG recordings at Emory in CAD patients",
		Project == p$eps ~
			"TBD",
		TRUE ~ Status
	)) |>
...
```{r}
#| label: Week 6 updates
updates <-
	updates |>
	mutate(Status = case_when(
		Project == p$mental_stress ~
			"Team = Amit Shah (*Emory*)  
			Draft to co-authors **8/9**",
		Project == p$af_registry ~
			"Complete AF registry for catheter ablation data dictionary **8/2**",
		Project == p$pert ~ 
			"Plan for IM Conference and Survey **8/10**",
		Project == p$grants ~ 
			"Aim 1 = Human vagolysis and SCD  
			**Aim 2 = Murine model of vagal withdrawal and arrhythmia**  
			Active research into model development **daily**",
		...
		Project == p$pixel ~
			"Team = Lee Frazin, Satish Mishra, David Polman
			Revise still images to create standard for processing and SOP **8/11**",
		Project == p$endocarditis ~
			"Meet with Atreya Mishra to complete outline **8/12**",
		Project == p$modeling ~
			"Forest plots by subgroups in {R}, **complete**  
			Revise multiple modeling approach in formulas",
		Project == p$signals ~
			"CARTO into OpenEP **8/9**",
		Project == p$biobank ~
			"Emory Biobank continually recruiting  
			Meet with staff to assess data quality **TBD**",
		Project == p$eps ~
			"EP lab on **8/8, 8/10, 8/11**",
		TRUE ~ Status
	)) |>
	mutate(Product = case_when(
		Project == p$mental_stress ~ 
...
```{r}
#| label: Week 7 updates
updates <-
	updates |>
	mutate(Status = case_when(
		Project == p$mental_stress ~
			"Team = Amit Shah (*Emory*)  
			Draft to co-authors **8/16**",
		Project == p$af_registry ~
			"AF registry needs revamp",
		Project == p$pert ~ 
			"Plan for IM Conference and Survey **8/18**",
		Project == p$grants ~ 
			"Aim 1 = Human vagolysis and SCD  
			**Aim 2 = Murine model of vagal withdrawal and arrhythmia**  
			Active research into model development **daily**",
		...
		Project == p$pixel ~
			"Team = Lee Frazin, Satish Mishra, David Polman
			Images collected for 3/10 patients, finish patients **8/16**",
		Project == p$endocarditis ~
			"Meet with Atreya Mishra to complete outline **8/18**",
		Project == p$modeling ~
			"On hold",
		Project == p$signals ~
			"LSPro and CARTO into Matlab, **8/19**",
		Project == p$biobank ~
			"Emory Biobank continually recruiting  
			Meet with staff to assess data quality **TBD**",
		Project == p$eps ~
			"EP lab scheduled TBD",
		TRUE ~ Status
	)) |>
	mutate(Product = case_when(
		Project == p$mental_stress ~ 
	...
	mutate(Effort = case_when(
		Project == p$mental_stress ~ 0.2,
		Project == p$af_registry ~ 0.3, 
		Project == p$pert ~ 0.05,
		Project == p$grants ~ 0.1, 
		Project == p$study ~ 0.01,
		Project == p$clinic ~ 0.1, 
		Project == p$pixel ~ 0.1,
		Project == p$endocarditis ~ 0.01,
...
```{r}
#| label: Week 8 updates
updates <-
	updates |>
	mutate(Status = case_when(
		Project == p$mental_stress ~
			"Team = Amit Shah (*Emory*)  
			Final draft for co-authors **8/22**",
		Project == p$af_registry ~
			"Team = Shaurya, Hatem, Malek,
			Complete review 200 charts to add ablation data **8/26**  
			Complete 50 of 100 current UIC ablation patients in REDCap **8/26**  
			Provide SOP for data collection **8/22**",
		Project == p$pert ~ 
			"Plan for IM Conference and Survey **8/24**",
		Project == p$grants ~ 
			"Aim 1 = Human vagolysis and SCD  
			**Aim 2 = Murine model of vagal withdrawal and arrhythmia**  
			Active research into model development **daily**",
		...
		Project == p$pixel ~
			"Team = Lee Frazin, Satish Mishra, David Polman
			Images collected for 3/10 patients, finish patients **8/26**",
		Project == p$endocarditis ~
			"Meet with Atreya **TBD**",
		Project == p$modeling ~
			"On hold",
		Project == p$signals ~
			"LSPro and CARTO into Matlab **success**",
		Project == p$biobank ~
			"Emory Biobank continually recruiting  
			Meet with staff to assess data quality **TBD**",
		Project == p$eps ~
...
```{r}
dt <- date_build(year = 2022, month = 09, day = 5)

tbl <-
	log |>
	filter(Start >= dt) |>
	filter(Start > dt & End < add_days(dt, 7)) |>
	inner_join(prog, by = "Type") |>
	select(Type, Project, Title, Description, Status, Duration)
```
...
```{r}
dt <- 
	date_build(year = 2022, month = 09, day = 19) |>
	add_days(-7)
...
effort <-
	...
	mutate(Effort = paste0(signif(as.numeric(Effort)/0.4, 3), "%")) 
