...
#| echo: false
dt <- 
	date_build(year = 2024, month = 2, day = 12) |>
	add_days(-30)
...
```{r}
#| label: tbl-afeqt-desc
#| tbl-cap: AFEQT/SDOH sample population
afeqt_tbls <- targets::tar_read(afeqt_tbls, store = '~/OneDrive - University of Illinois Chicago/targets/aflubber')
afeqt_tbls$desc 
```
...
```{r}
#| label: tbl-afeqt-scores
#| tbl-cap: AFEQT/SDOH scores
afeqt_tbls$afeqt
```
...
```{r}
#| label: tbl-afeqt-baseline
#| tbl-cap: AFEQT/SDOH baseline characteristics
afeqt_tbls$baseline
```
...
```{r}
#| label: tbl-afeqt-followup
#| tbl-cap: AFEQT/SDOH follow-up characteristics
afeqt_tbls$followup
```
