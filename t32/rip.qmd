...
#| echo: false
dt <- 
	date_build(year = 2023, month = 7, day = 24) |>
	add_days(-30)
...
```{r}
af <- 
	vroom::vroom("~/projects/cbcd/output/ECG-AFibDiagnosis.tsv") |>
	mutate(DATETIME = as.POSIXct(paste(DATE, TIME), format = "%m/%d/%Y %H:%M:%S")) |>
	ggplot(data = _, aes(x = DATETIME)) + geom_histogram()

ecg <- 
	vroom::vroom("~/projects/cbcd/output/ECG-PatientList.tsv") |>
	mutate(DATETIME = as.POSIXct(paste(DATE, TIME), format = "%m/%d/%Y %H:%M:%S")) |>
	ggplot(data = _, aes(x = DATETIME)) + geom_histogram()
